...
uniform float u_blind_freq = 90.0f;
...
void main(void) {   
	...
	if( u_screen_effect ){
		float x_mod, y_mod;
		x_mod = mod(v_position_sc.x*(u_screen_count+1.0f), 1.0f);
		y_mod = mod(v_position_sc.y*u_screen_count, 1.0f);
		if( (x_mod > u_screen_width) && (x_mod < 1.0f-u_screen_width) &&
